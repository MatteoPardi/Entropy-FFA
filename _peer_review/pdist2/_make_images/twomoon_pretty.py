...
class TwoMoon:
    ...
    def plot (self, x=None, y=None, moon=True): #pretty
        
        plt.figure(figsize=(3,3))
        if x is not None and y is not None:
            x0 = x[np.argwhere(y.flatten() == 0).flatten()]
            x1 = x[np.argwhere(y.flatten() == 1).flatten()]
            plt.plot(x0[:,0], x0[:,1], ls='', marker='s', color='midnightblue', alpha=0.6)
            plt.plot(x1[:,0], x1[:,1], ls='', marker='s', color='mistyrose', alpha=0.6)
        if moon:
            x0 = self.x[np.argwhere(self.y.flatten() == 0).flatten()]
            x1 = self.x[np.argwhere(self.y.flatten() == 1).flatten()]
            plt.plot(x0[:,0], x0[:,1], ls='', marker='.', ms=3.5, color='red')
            plt.plot(x1[:,0], x1[:,1], ls='', marker='.', ms=3.5, color='blue')
        plt.xlim(-2.067,2.067)
        plt.ylim(-1.55,1.55)
        plt.xticks([])
        plt.yticks([])
        plt.tight_layout()
    ...
    def plot_proba (self, x=None, y=None, moon=True): #pretty
